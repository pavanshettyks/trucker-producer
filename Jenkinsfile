node{
    def DOCKERHUB_REPO = "pavanshettyks/trcuker-producerr"
   ...
   try{
    sh "docker rm -f TruckerProducer || true "
    sh "docker run --env-file /prop.env -p 8080:8080 -d --name TruckerProducer ${DOCKERHUB_REPO}:${DOCKER_IMAGE_VERSION}"
   }
    catch (e) {
